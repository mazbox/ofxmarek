...
//------------------------------------------------------------------
ofxTrueTypeFont::ofxTrueTypeFont(){
	bLoadedOk		= false;
	bMakeContours	= false;
	letterSpacing	= 1.f;
	wordSpacing		= 1.f;
...
//------------------------------------------------------------------
void ofxTrueTypeFont::loadFont(string filename, int fontsize){
	// load anti-aliased, non-full character set:
	loadFont(filename, fontsize, true, false, false);
}

//------------------------------------------------------------------
void ofxTrueTypeFont::loadFont(string filename, int fontsize, bool _bAntiAliased, bool _bFullCharacterSet, bool makeContours){
...
//-----------------------------------------------------------
int ofxTrueTypeFont::ofNextPow2 ( int a )
{
	int rval=1;
	while(rval<a) rval<<=1;
	return rval;
...
//-----------------------------------------------------------
void ofxTrueTypeFont::setLineHeight(float _newLineHeight) {
	lineHeight = _newLineHeight;
}

//-----------------------------------------------------------
float ofxTrueTypeFont::getLineHeight(){
	return lineHeight;
}

//------------------------------------------------------------------
ofTTFCharacter ofxTrueTypeFont::getCharacterAsPoints(int character){
...
//-----------------------------------------------------------
void ofxTrueTypeFont::drawChar(int c, float x, float y) {
...
//-----------------------------------------------------------
float ofxTrueTypeFont::stringWidth(string c) {
    ofRectangle rect = getStringBoundingBox(c, 0,0);
    return rect.width;
}

float ofxTrueTypeFont::getDescenderHeight() {
	int g = (unsigned char)'g' - NUM_CHARACTER_TO_START;
	int a = (unsigned char)'a' - NUM_CHARACTER_TO_START;
	return float(cps[g].height - cps[a].height);
}

ofRectangle ofxTrueTypeFont::getStringBoundingBox(string c, float x, float y){
		...
		int cy = (unsigned char)c[index] - NUM_CHARACTER_TO_START;
 	    if (cy < nCharacters){ 			// full char set or not?
			if (c[index] == '\n') {
				...
				xoffset += (float)cps[cy].width*(float)wordSpacing;
				...
				xoffset += (float)cps[cy].setWidth*(float)letterSpacing;
...
//-----------------------------------------------------------
float ofxTrueTypeFont::stringHeight(string c) {
    ofRectangle rect = getStringBoundingBox(c, 0,0);
    return rect.height;
}

//=====================================================================
void ofxTrueTypeFont::drawString(string c, float x, float y) {
