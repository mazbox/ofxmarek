...
class GuiContour: public GuiControl {
public:
	...
	void load() {
		bgImg = resources->getImage(bgImgUrl);
		pointImg = resources->getImage(pointImgUrl);
		pointDownImg = resources->getImage(pointDownImgUrl);
	...
	virtual vector<ParameterInfo> getParameterInfo() {
		...
		params.push_back(ParameterInfo("Point Color", "pointColor", "colorpicker", &pointColor));
