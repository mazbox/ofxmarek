...
public:
	...
	void load() {
		toggleOn = gui::Resources::getImage(toggleOnUrl);
		toggleOff = gui::Resources::getImage(toggleOffUrl);
		toggleDown = gui::Resources::getImage(toggleDownUrl);
		if(toggleOn!=NULL) {
			scalable = false;
	...
	void getParameterInfo(vector<ParameterInfo> &params) {
		...
		params.push_back(ParameterInfo("Stroke Color", "strokeColor", "colorpicker", &strokeColor));
