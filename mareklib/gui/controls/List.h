...
public:
	...
	void draw() {
		...
		for(int i =0; i < items.size(); i++) {
			...
			gui::Resources::drawString(items[i], x+5, scrollOffset + y + (i+1)*itemHeight-4);
		...
		gui::Resources::drawString("UP", 4+x+width-scrollerWidth, y+height/2 - 5);
		gui::Resources::drawString("DN", 4+x+width-scrollerWidth, y+height/2 + 15);
		maskOff();
		ofFill();
		drawLabel();
	}
