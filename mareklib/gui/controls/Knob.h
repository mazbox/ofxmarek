...
public:
	...
	Knob(): LabeledControl() {
		...
		bgColor = 0x505050;
		fgColor = 0x960000;
		maxAngle = 330*PI/180.f;
		minAngle = 30*PI/180.f;
	...
	void load() {
		...
		bgImage = gui::Resources::getImage(bgImageUrl);
		needleImage = gui::Resources::getImage(needleImageUrl);
		if(needleImage!=NULL) {
	...
	void draw() {
		...
		if(needleImage!=NULL) {
			...
			setRGBA(fgColor);
			ofLine(center.x, center.y, center.x + radius*cos(angle), center.y + radius*sin(angle));	
	...
	bool touchMoved(int _x, int _y, int touchId) {
		ofPoint fromCenter = ofPoint((_x-x)-(width/2), (_y-y)-(width/2));
		float newTouchAngle = atan2(fromCenter.y, fromCenter.x);
