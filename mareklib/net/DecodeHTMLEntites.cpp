/*
 *  DecodeHTMLEntites.cpp
 *  Ticker
 *
 *  Created by Marek Bereza on 23/08/2010.
 *  Marek found this stuff somewhere on the internet, sorry can't remember where
 *
 */

#include "DecodeHTMLEntites.h"
...
string decodeHTMLEntities(string input) {
	char *dest = new char[input.size()+20];
	decode_html_entities_utf8(dest, input.c_str());
	string toReturn = string(dest);
	delete [] dest;
	return toReturn;
}
/*

// Fixed entities decoding
#include <boost/algorithm/string.hpp>
std::string decodeHTMLEntitiesNoAmp(std::string input) 
{
	std::string tmp = input;
	
	// go through all entities and replace them with UTF8 counterparts
	//int numEntities = sizeof(named_entities)/2;
	int numEntities = 252;
	for(int i=0; i<numEntities; i++) {
		std::string from = std::string(named_entities[i][0]);
		std::string to = std::string(named_entities[i][1]);
		boost::replace_all(tmp, from, to);
	}
